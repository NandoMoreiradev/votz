...
@Processor(SENADO_SYNC_QUEUE, { concurrency: 1 })
export class SenadoSyncProcessor extends WorkerHost {
  private readonly logger = new Logger(SenadoSyncProcessor.name)
    ...
    switch (job.name) {
      case 'full-sync': {
        ...
        this.logger.log(
          ...
          `ignorados: ${result.skipped} (${result.finishedAt.getTime() - result.startedAt.getTime()}ms)`,
        )
        return
      }
